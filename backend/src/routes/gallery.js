...
// GET /api/g/:token — public gallery view (own photos + subfolders to browse into)
router.get('/:token', async (req, res) => {
  try {
    const folder = await prisma.folder.findUnique({
      where:   { shareToken: req.params.token },
      include: { children: { orderBy: { createdAt: 'asc' }, include: { _count: { select: { images: true } } } } }
    })
    ...
    const ownImages = await prisma.image.findMany({
      where:   { folderId: folder.id },
      orderBy: { uploadedAt: 'asc' }
    })

    const images = ownImages.map(img => ({
      id:         img.id,
