    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="pv-body text-[var(--pv-muted)] mt-1">Overview of all your scheduled weddings and events</p>
        </div>
        <button
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        <div className="flex gap-1 bg-[var(--pv-card)] border border-[var(--pv-border)] rounded-xl p-1">
          {[
            { key: 'all',       label: 'All' },
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${
                filter === tab.key
                  ? 'bg-[var(--pv-accent)] text-[var(--pv-accent-on)]'
                  : 'text-[var(--pv-text-secondary)] hover:text-[var(--pv-text)]'
              }`}
            >
              {tab.label}
            </button>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2 border border-[var(--pv-border)] rounded-xl text-sm text-[var(--pv-text)] bg-[var(--pv-card)] focus:outline-none focus:border-[var(--pv-accent)] transition-all w-64"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div key={stat.label} className="bg-[var(--pv-card)] border border-[var(--pv-border)] p-5">
            <p className="pv-metric">{stat.value}</p>
            <p className="pv-label mt-1">{stat.label}</p>
          </div>
        ))}
      </div>

      <h2 className="pv-section-title mb-5">
      ...
      {loading ? (
        ...
        <div className="text-center py-24 bg-[var(--pv-card)] border border-[var(--pv-border)] rounded-2xl">
          <div className="w-14 h-14 bg-[var(--pv-accent-tint)] rounded-2xl flex items-center justify-center mx-auto mb-4">
            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="var(--pv-accent)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
          </div>
          <p className="text-[var(--pv-muted)]">No events found. Create your first wedding event.</p>
          <button
            onClick={() => router.push('/admin/crm/new')}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {upcomingFirst.map(event => (
            <div key={event.id}
              className="bg-[var(--pv-card)] border border-[var(--pv-border)] rounded-2xl overflow-hidden hover:shadow-lg transition-all">
              ...
              <div className="p-5">
                <h3 className="text-lg font-bold text-[var(--pv-text)] mb-3">
                  {event.coupleName}
                </h3>
                {event.location && (
                  <div className="text-sm text-[var(--pv-muted)] mb-2 truncate">{event.location}</div>
                )}
                 <div className="text-sm text-[var(--pv-muted)] mb-4">
                  {formatDate(event.startDate)} – {formatDate(event.endDate)}
                 </div>
                <div className="flex items-center justify-between pt-3 border-t border-[var(--pv-border)]">
                  <button
                    onClick={() => router.push(`/admin/crm/${event.id}`)}
                    className="flex items-center gap-2 px-4 py-2 border border-[var(--pv-border)] rounded-lg text-sm text-[var(--pv-text-secondary)] hover:border-[var(--pv-accent)] hover:text-[var(--pv-accent)] transition-all"
                  >
                     Edit Event
                  </button>
                  <button
                    onClick={() => router.push(`/admin/crm/${event.id}`)}
                    className="text-[var(--pv-muted)] hover:text-[var(--pv-text-secondary)] transition-all text-lg px-2"
                  >
