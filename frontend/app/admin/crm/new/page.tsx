...
export default function NewWedding() {
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'var(--pv-accent-tint-hover)' }}>
      ...
      {/* Header */}
      <div className="bg-white border-b border-[var(--pv-accent-tint)] px-8 py-5 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[var(--pv-muted)] text-sm">Create a new wedding and schedule events</p>
        </div>
      </div>
      ...
      <div className="p-8 grid grid-cols-5 gap-6 max-w-7xl mx-auto">
        ...
        <div className="col-span-2 bg-white rounded-2xl border border-[var(--pv-accent-tint)] p-6 h-fit">
          <h2 className="text-lg font-bold text-[var(--pv-text)] mb-1">1. Wedding Details</h2>
          <p className="text-[var(--pv-muted)] text-sm mb-6">Basic information about the couple</p>

          <div className="flex flex-col gap-4">
            <div>
              <label className="block text-sm font-medium text-[var(--pv-text-secondary)] mb-1.5">Couple Name *</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-[var(--pv-text-secondary)] mb-1.5">Start Date *</label>
                <input type="date" value={startDate}
                  onChange={e => { setStartDate(e.target.value); setDetailsSaved(false) }}
                  className="w-full px-3 py-2.5 border border-[var(--pv-border)] rounded-xl text-sm text-[var(--pv-text)] focus:outline-none focus:border-[var(--pv-accent)] transition-all"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-[var(--pv-text-secondary)] mb-1.5">End Date *</label>
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--pv-text-secondary)] mb-1.5">
                Description <span className="text-[var(--pv-muted)] font-normal">(Optional)</span>
              </label>
              <textarea value={description}
                ...
                rows={3}
                className="w-full px-4 py-2.5 border border-[var(--pv-border)] rounded-xl text-sm text-[var(--pv-text)] focus:outline-none focus:border-[var(--pv-accent)] transition-all resize-none"
              />
            </div>
            ...
            <button onClick={saveDetails}
              ...
              className={`w-full py-3 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2 ${
                detailsSaved
                  ...
                  : 'bg-[var(--pv-accent)] text-[var(--pv-accent-on)] hover:bg-[var(--pv-accent-hover)] disabled:opacity-40'
              }`}
            >
        ...
        <div className="col-span-3 bg-white rounded-2xl border border-[var(--pv-accent-tint)] p-6">
          <h2 className="text-lg font-bold text-[var(--pv-text)] mb-1">2. Event Schedule</h2>
          <p className="text-[var(--pv-muted)] text-sm mb-6">Plan events and assign photographers for each day</p>
            ...
            <>
                ...
                {Array.from({ length: getDayCount() }).map((_, i) => (
                  <button key={i} onClick={() => setActiveDay(i)}
                    className={`flex flex-col items-center px-4 py-2.5 rounded-xl text-sm font-medium whitespace-nowrap transition-all border ${
                      activeDay === i
                        ? 'bg-[var(--pv-ink)] text-white border-[var(--pv-ink)]'
                        : 'bg-white text-[var(--pv-text-secondary)] border-[var(--pv-border)] hover:border-[var(--pv-accent)]'
                    }`}
                  >
                    <span className="font-semibold">Day {i + 1}</span>
                    <span className={`text-xs mt-0.5 ${activeDay === i ? 'text-white/60' : 'text-[var(--pv-muted)]'}`}>
                      {getDayDate(i)}
                    </span>
                  </button>
                ))}
                <button className="flex flex-col items-center px-4 py-2.5 rounded-xl text-sm border border-dashed border-[var(--pv-accent)] text-[var(--pv-accent)] hover:bg-[var(--pv-accent-tint-hover)] transition-all whitespace-nowrap">
                  <span>+</span>
                  ...
                  return (
                    <button key={slot.key} onClick={() => setActiveSlot(slot.key)}
                      className={`flex flex-col items-center py-4 px-3 rounded-xl border transition-all ${
                        isActive    ? 'bg-[var(--pv-accent)] text-[var(--pv-accent-on)] border-[var(--pv-accent)]'
                        : hasBooking ? 'bg-[#f0fdf4] text-green-700 border-green-200'
                        : 'bg-[#fafafa] text-[var(--pv-text-secondary)] border-[var(--pv-border)] hover:border-[var(--pv-accent)]'
                      }`}
                    >
                      <span className="font-semibold text-sm">{slot.label}</span>
                      <span className={`text-xs mt-0.5 ${isActive ? 'text-white/70' : 'text-[var(--pv-muted)]'}`}>
                        {slot.time}
                      </span>
                    </button>
                  )
              ...
              <div className="border-t border-[var(--pv-border)] pt-5">
                <div className="flex items-center gap-2 mb-4">
                  <div className="w-1 h-5 bg-[var(--pv-accent)] rounded-full"></div>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[var(--pv-text-secondary)] mb-1.5">Assign Photographers *</label>
                    <div className="border border-[var(--pv-border)] rounded-xl p-3 min-h-[46px]">
                      {selectedPhotographers.length > 0 && (
                        <div className="flex flex-wrap gap-2 mb-2">
                          {selectedPhotographers.map(id => {
                            const p = photographers.find(ph => ph.id === id)
                            return (
                              <div key={id} className="flex items-center gap-1.5 bg-[var(--pv-accent-tint)] text-[var(--pv-accent)] px-3 py-1 rounded-full text-xs font-medium">
                                <span>{p?.name}</span>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[var(--pv-text-secondary)] mb-1.5">Location</label>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    ...
                    <div className="flex gap-3">
                      <button onClick={() => handleBook(false)}
                        ...
                        className="px-5 py-2.5 border border-[var(--pv-border)] text-sm font-semibold text-[var(--pv-text-secondary)] rounded-xl hover:border-[var(--pv-accent)] hover:text-[var(--pv-accent)] disabled:opacity-40 transition-all"
                      >
                        Book
                      </button>
