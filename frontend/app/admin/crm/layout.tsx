    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2.5 bg-[#c8a020] text-white text-sm font-semibold rounded-xl hover:bg-[#b8901a] transition-all shadow-md"
        >
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        <div className="flex gap-1 bg-white border border-[#e8e5e0] rounded-xl p-1">
          {[
            { key: 'all',       label: 'All' },
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2 border border-[#e8e5e0] rounded-xl text-sm text-[#0f0f0f] bg-white focus:outline-none focus:border-[#c8a020] transition-all w-64"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'Upcoming',        value: events.filter(e => getStatus(e) === 'upcoming').length,  color: 'bg-white' },
          { label: 'Active',          value: events.filter(e => getStatus(e) === 'active').length,    color: 'bg-white' },
          { label: 'Completed',       value: events.filter(e => getStatus(e) === 'completed').length, color: 'bg-white' },
        ].map(stat => (
          <div key={stat.label} className="bg-white border border-[#e8e5e0] rounded-2xl p-5">
            <p className="text-3xl font-bold text-[#0f0f0f]">{stat.value}</p>
            <p className="text-sm text-[#888] mt-1">{stat.label}</p>
          </div>
        ))}
      </div>

      {/* Upcoming Weddings heading */}
      ...
      {loading ? (
        ...
        <div className="text-center py-24 bg-white border border-[#e8e5e0] rounded-2xl">
          ...
          <button
            onClick={() => router.push('/admin/crm/new')}
            className="mt-4 px-6 py-2.5 bg-[#c8a020] text-white text-sm font-semibold rounded-xl hover:bg-[#b8901a] transition-all"
          >
            + New Wedding
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {upcomingFirst.map(event => (
            <div key={event.id}
              className="bg-white border border-[#e8e5e0] rounded-2xl overflow-hidden hover:shadow-md transition-all">
              ...
              <div className="p-5">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-[#f0ede8]">
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 border border-[#e8e5e0] rounded-lg text-sm text-[#333] hover:border-[#c8a020] hover:text-[#c8a020] transition-all"
                  >
                    ✏️ Edit Event
                  </button>
                  <button
